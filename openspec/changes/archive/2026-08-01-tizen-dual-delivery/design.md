...
Web TV runtime compat (chrome85 downlevel / polyfills / `/tv` `no-store`) is a separate concern tracked in [#6](https://github.com/viehao/vkara/issues/6).
...
- Generated `dist/tizenbrew/package.json` MUST include `"name": "@vkara/tv"`, matching semver, `"publishConfig": { "access": "public" }`, and `repository.url` → `https://github.com/viehao/vkara`.
...
| Trusted Publisher | GitHub `viehao/vkara`, workflow filename **exactly** `release-tizen.yml` |
