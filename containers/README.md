...
| ------------------------------ | ----------------------------------- | ------------- | ---------------------------------------------------------------- |
| `viehao/vkara-api`       | `apps/api/Dockerfile`               | `8000`        | API behind your own Redis / reverse proxy                        |
| `viehao/vkara-web`       | `apps/web/Dockerfile`               | `3000`        | Frontend only; point env at external API                         |
| `viehao/vkara-api-redis` | `containers/api-redis/Dockerfile`   | `8000`        | Backend bundle without Next.js; Redis stays inside the container |
| `viehao/vkara-aio`       | `containers/aio/Dockerfile`         | `3000`        | Single container, one public port                                |
...
```bash
docker pull viehao/vkara-aio:latest
docker run --rm -p 3000:3000 viehao/vkara-aio:latest
```
...
`viehao/vkara-aio` for single-container self-hosting. Split `vkara-api` and `vkara-web` when you run your own reverse proxy or scale components separately. Use `vkara-api-redis` for backend-only.
...
See `containers/api-redis/.env.example`. Redis password defaults to `viehao` in supervisord (change for production).
...
Redis credentials are fixed inside the image (`127.0.0.1:6379`, password `viehao`).
