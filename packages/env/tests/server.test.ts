...
describe('isCorsOriginAllowed', () => {
    ...
    it('allows only listed origins when CORS_ORIGINS is set', () => {
        const corsOrigins = 'https://vkara-local.viehao.io.vn,http://localhost:3000';
        expect(isCorsOriginAllowed('https://vkara-local.viehao.io.vn', corsOrigins)).toBe(true);
    ...
    it('reflects allowed origin on upgrade', () => {
        expect(
            resolveWsUpgradeCorsHeaders(
                'https://vkara-local.viehao.io.vn',
                'https://vkara-local.viehao.io.vn',
            ),
        ).toEqual({
            'access-control-allow-origin': 'https://vkara-local.viehao.io.vn',
            vary: 'Origin',
